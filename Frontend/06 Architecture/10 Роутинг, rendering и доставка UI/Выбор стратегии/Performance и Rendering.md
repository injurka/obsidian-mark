...
**Паттерн: Resumability (Оптимизация гидратации на примере Qwik)**
Qwik переворачивает игру: он не гидратирует приложение, он "возобновляет" (resumes) его выполнение с того места, где остановился сервер.
```javascript
import { component$, $, useSignal } from '@builder.io/qwik';

export const Counter = component$(() => {
  const count = useSignal(0); // Стейт будет сохранен сервером в HTML атрибуты
  
  // Символ $ говорит компилятору: вынести эту функцию в отдельный микро-бандл.
  // Эта функция скачается ТОЛЬКО если пользователь физически кликнет на кнопку.
  // До клика — на клиенте исполняется 0 байт JavaScript кода.
