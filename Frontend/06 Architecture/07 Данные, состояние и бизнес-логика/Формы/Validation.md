...
## Подходы к реализации: Нативная HTML5 vs JavaScript

Первый слой валидации можно получить вообще без JS — через **Constraint Validation API**. Браузер сам валидирует атрибуты `required`, `type="email"`, `minlength`, `pattern` и блокирует отправку формы с нативной подсказкой.

```html
<input type="email" required minlength="5" pattern=".+@company\.com" />
```
...
input.addEventListener('blur', () => {
  if (!input.validity.valid) {
    // Кастомное сообщение вместо системного
    input.setCustomValidity('Введите корпоративную почту');
    input.reportValidity(); // Показать подсказку немедленно
  }
});
```

*Плюсы:* работает без JS, бесплатная доступность (скринридеры понимают нативные ошибки), быстро для простых форм.
*Минусы:* системные подсказки нельзя нормально стилизовать, тексты зависят от локали браузера (русский пользователь с английским Chrome увидит ошибку по-английски), сложные кросс-полевые правила («пароли совпадают») не выразить. Поэтому в SPA почти всегда используют JS-валидацию (схемы), а HTML5-атрибуты оставляют как прогрессивное улучшение и семантику.
