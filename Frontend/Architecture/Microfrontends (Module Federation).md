# Микрофронтенды и Webpack Module Federation
...
```text
[Host Application (Контейнер)]
  │
  ├──► Динамический импорт по сети ──► [Remote App: Личный Кабинет] (deploy-server-1)
  └──► Динамический импорт по сети ──► [Remote App: Корзина/Оплата] (deploy-server-2)
```
