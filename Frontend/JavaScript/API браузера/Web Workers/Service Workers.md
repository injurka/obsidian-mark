...
## Экосистема Background APIs (Фоновые API)

Service Workers — это лишь фундамент. На его базе работают несколько мощнейших API, стирающих грань между вебом и нативом.

### 1. Background Fetch API
Позволяет скачивать (или загружать) тяжелые файлы в фоне.
**Проблема, которую решает:** Обычный `fetch` прервется, если пользователь закроет вкладку или потеряет сеть на пару минут.
**Как работает Background Fetch:**
* Запрос передается самому браузеру (а не странице).
* Браузер показывает системное уведомление ОС с прогресс-баром загрузки.
* Если интернет пропал, браузер ставит загрузку на паузу и сам возобновит её при появлении сети.
* Когда загрузка завершена, браузер будит ваш Service Worker событием `backgroundfetchsuccess`.

**Пример кода (в основном потоке):**
```javascript
navigator.serviceWorker.ready.then(async (swRegistration) => {
  const bgFetch = await swRegistration.backgroundFetch.fetch(
    'my-podcast-download',
    ['/audio/episode1.mp3', '/images/cover.jpg'],
    {
      title: 'Скачивание подкаста',
      icons: [{ src: '/icons/podcast.png', sizes: '192x192' }],
      downloadTotal: 15000000 // Для точного прогресс-бара
    }
  );
});
```

### 2. Background Sync API (Фоновая синхронизация)
**Сценарий:** Пользователь пишет сообщение в мессенджере (вашем PWA), находясь в метро без интернета. Он нажимает "Отправить" и закрывает приложение.
**Решение:**
1. Приложение сохраняет сообщение локально (например, в IndexedDB).
2. Приложение регистрирует событие синхронизации: `swRegistration.sync.register('send-messages')`.
3. Как только пользователь выходит из метро и появляется интернет (даже если PWA уже закрыто), браузер будит Service Worker и вызывает событие `sync`.
4. В обработчике `sync` Service Worker достает сообщение из IndexedDB и отправляет его на сервер через `fetch`.

### 3. Periodic Background Sync API
Позволяет приложению "просыпаться" в фоне по расписанию для обновления данных.
**Сценарий:** Новостной сайт каждое утро в 7:00 просыпается и скачивает свежие статьи в кэш. Когда пользователь открывает приложение за завтраком, статьи грузятся мгновенно и без интернета.
**Ограничения (важно!):**
Чтобы не посадить батарею устройства, браузеры используют сложную эвристику (Site Engagement Score). Периодическая синхронизация сработает только если:
* Приложение установлено как PWA.
* Пользователь активно и часто им пользуется.
* Устройство подключено к Wi-Fi и, желательно, к зарядке (зависит от ОС).
