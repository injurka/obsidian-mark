...
Для реализации полноценных уведомлений (как в нативных приложениях) используются два связанных, но разных API:

### 1. Notifications API
Отвечает исключительно за *отображение* уведомлений на экране устройства пользователя. Скрипт страницы или Service Worker может вызвать это API, чтобы показать пользователю сообщение, заголовок и иконку.

### 2. Push API
Отвечает за *доставку* сообщений от вашего сервера к браузеру пользователя, даже когда ваш сайт закрыт и пользователь не взаимодействует с ним.
*   Опирается на Service Workers, которые просыпаются при получении push-сообщения от браузерного push-сервиса (например, FCM для Chrome).
*   Получив push-событие, Service Worker обычно использует Notifications API, чтобы показать уведомление пользователю.
