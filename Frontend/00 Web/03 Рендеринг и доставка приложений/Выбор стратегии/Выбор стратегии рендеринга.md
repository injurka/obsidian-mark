...
**Антипаттерн: SSG для миллиона комбинаций URL**
```javascript
...
### Nuxt 3 / Vue 3 (Гибридный рендеринг через `routeRules`)

Главная фича Nuxt 3 — возможность комбинировать ВСЕ стратегии рендеринга в едином приложении без разделения на разные проекты:

```typescript
// nuxt.config.ts
export default defineNuxtConfig({
  // Централизованное дерево правил гибридного рендеринга:
  routeRules: {
    // 1. SSG / Prerender: Главная и статические страницы (генерация при билде)
    '/': { prerender: true },
    '/about': { prerender: true },

    // 2. SWR / ISR: Карточки товаров кэшируются на 1 час, обновление в фоне
    '/products/**': { swr: 3600 },

    // 3. SSR по умолчанию: Динамические новости и статьи
    '/news/**': { ssr: true },

    // 4. CSR / SPA: Личный кабинет и админка рендерятся строго на клиенте
    '/admin/**': { ssr: false },
    '/dashboard/**': { ssr: false },

    // 5. Caching & CORS headers на Edge
