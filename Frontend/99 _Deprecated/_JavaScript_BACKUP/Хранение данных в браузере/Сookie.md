...
## Cookies (Куки)
**Cookies** — это небольшие фрагменты данных, отправляемые веб-сервером и сохраняемые на компьютере пользователя веб-браузером. В отличие от других хранилищ, куки автоматически прикрепляются к каждому HTTP-запросу к серверу, который их установил.

*   **Применение:** Управление сессиями (логины, корзины), персонализация (настройки темы), трекинг (аналитика, рекламные идентификаторы).
*   **Размер:** Ограничены примерно 4 КБ на одну куку.
*   **Безопасность и атрибуты:**
    *   `Secure`: Передаются только по HTTPS.
    *   `HttpOnly`: Недоступны из JavaScript (защита от XSS).
    *   `SameSite`: Защита от CSRF-атак (`Strict` или `Lax`).
