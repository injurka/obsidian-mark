...
Оба механизма (часть Web Storage API) предоставляют браузеру возможность синхронно хранить пары ключ-значение. Они гораздо проще и быстрее в использовании, чем IndexedDB, но подходят только для небольших объемов строковых данных (обычно лимит ~5 МБ).

*   **Local Storage (`localStorage`):** Данные хранятся бессрочно и не удаляются при закрытии браузера. Доступны для всех вкладок/окон с тем же origin (домен + протокол + порт).
*   **Session Storage (`sessionStorage`):** Данные живут только пока открыта вкладка. Если вкладку закрыть — данные удаляются. Каждая вкладка имеет свое изолированное хранилище, даже если открыт один и тот же сайт.
