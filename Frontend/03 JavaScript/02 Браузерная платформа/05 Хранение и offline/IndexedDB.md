...
## IndexedDB
**IndexedDB** — это низкоуровневое API для клиентского хранения значительных объемов структурированных данных (включая файлы и blobs). Это полноценная асинхронная NoSQL база данных прямо в браузере.

*   **Особенности:** В отличие от Web Storage, работает асинхронно (не блокирует главный поток), поддерживает транзакции, индексы и курсоры для эффективного поиска.
*   **Применение:** Оффлайн-приложения, кэширование больших объемов данных (статьи, каталоги товаров), работа с тяжелыми объектами, где 5 МБ лимита Local Storage недостаточно.
*   **Сложность:** Нативный API IndexedDB считается сложным и многословным. На практике часто используются библиотеки-обертки (например, `idb`, `Dexie.js`, `localForage`), которые предоставляют более удобный интерфейс на основе промисов.
