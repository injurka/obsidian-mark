...
`Symbol.iterator` — это, пожалуй, наиболее известный символ. Он позволяет определять, как объект должен перебираться с помощью инструкции [[Цикл for...of|for...of]] или [[spread|spread-оператора]] (и должен ли он перебираться вообще).  
...
Объект, в котором определяется метод Symbol.iterator, должен соответствовать [[Протокол итератора|протоколу перебора (итератора)]].
...
Создание итерируемого объекта можно упростить с помощью [[Генераторы|функции-генератора]]. Данная функция возвращает объект `[Generator]`, соответствующий протоколу 
перебора:
```js
const myMethods = {
  toString: () => '[object myMethods]',
...
`*[Symbol.iterator]() { }` определяет метод класса — [[Генераторы|функцию-генератор]]. Экземпляр Fibonacci соответствует протоколу перебора. [[spread|spread-оператор]] вызывает метод **@@iterator** для создания массива чисел.
...
Рассматриваемое свойство определяется в таких типах, как [[Symbol]], [[Генераторы]], Map, Promise и др. Рассмотрим пример:
...
## Раздел из файла: `Прочее/Well-known Symbols (Прочее).md`
