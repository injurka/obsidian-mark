...
| **Ручной маршрут автобуса** | `> [!BUS]- Автобус` + строка системы | **Под активностью** | Остановки, номер маршрута, оператор, направление и короткий переход после высадки; одна строка таблицы = один отрезок | Chiayi Station → Hinoki Village, автобус 7313 |
...
3. **Автобусный callout `[!BUS]`:**
   * Размещается под активностью переезда. Таблица использует колонки `Откуда`, `Куда`, `Маршрут`, `Код`, `Оператор`, `Направление`, `Остановки` и `Пешком`.
   * Неизвестные сведения обозначаются `—`; короткий переход от остановки до точки назначения не смешивается с числом остановок.
   * Это vault-формат для чтения. Импортёр Trip Scheduler пока не преобразует `[!BUS]` в отдельную секцию транспорта, поэтому при проверке совместимости его нужно считать обычным callout.
...
## 4. Организация медиа-вложений (`00 - Файлы и документы/`)

Все скриншоты, фотографии и карты привязываются по структуре:
- `Personal Note/Travel/-- <Название>/00 - Файлы и документы/all/` — общие фото региона, обложки заметок (`note_*.jpg`).
- `Personal Note/Travel/-- <Название>/00 - Файлы и документы/01/`, `02/`... — фото к конкретным дням (`01_*.jpg`, `Pasted image *.png`).
