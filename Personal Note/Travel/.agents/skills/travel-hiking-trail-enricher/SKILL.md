...
description: >-
  Находит запланированные хайкинги, прогулочные тропы и веломаршруты и добавляет
  проверенные машиночитаемые ссылки для Trip Scheduler. Используй только когда
  маршрут существует и его доступность проверена на даты поездки.
---

# 🥾 Travel Hiking Trail Enricher: Добавление Хайкинг-Треков в Заметки

Находи только фактически запланированные пешие, горные и велосипедные маршруты и
дополняй их проверенными машиночитаемыми ссылками.
...
Сначала проверь официальный статус тропы, сезонность, разрешения, сложность,
набор высоты, время прохождения и транспорт к старту и финишу. Сохраняй источник
и дату проверки. Закрытую или неподтвержденную тропу не маскируй правдоподобной
ссылкой.
...
* **Последний фоллбэк:** Google Maps Directions используй для городской прогулки
  по дорогам. Не считай автоматически построенный маршрут надежным треком
  природной тропы. Если геометрия не подтверждена, оставь предупреждение:
...
3. **Для каждой активности без трека:**
   ...
   - Если маршрута нет в каталоге — проверь официальный источник или надежный
     трековый сервис. Google Maps генерируй только для подходящей дорожной сети.
...
- [ ] Проверены статус, сезонность, разрешения, сложность и дата источника
