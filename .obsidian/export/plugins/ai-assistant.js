...
var ve = (a, n, e) => n in a ? ye(a, n, { enumerable: !0, configurable: !0, writable: !0, value: e }) : a[n] = e;
var x = (a, n, e) => ve(a, typeof n != "symbol" ? n + "" : n, e), $e = (a, n, e) => n.has(a) || oe("Cannot " + e);
var ie = (a, n, e) => n.has(a) ? oe("Cannot add the same private member more than once") : n instanceof WeakSet ? n.add(a) : n.set(a, e);
var A = (a, n, e) => ($e(a, n, "access private method"), e);
function K() {
  return {
    async: !1,
    breaks: !1,
...
const pe = /[&<>"']/, Ee = new RegExp(pe.source, "g"), we = /[<>"']|&(?!(#\d{1,7}|#[Xx][a-fA-F0-9]{1,6}|\w+);)/, Te = new RegExp(we.source, "g"), Ne = {
  "&": "&amp;",
  "<": "&lt;",
  ">": "&gt;",
  '"': "&quot;",
...
function m(a, n) {
  ...
  const t = {
    replace: (i, r) => {
      ...
      return o = o.replace(Ce, "$1"), e = e.replace(i, o), t;
    },
...
`, n = n.substring(d.length + 1), u = !0), !u) {
          ...
          for (; n; ) {
            const Z = n.split(`
`, 1)[0];
            if (d = Z, this.options.pedantic && (d = d.replace(/^ {1,4}(?=( {4})*[^ ])/g, "  ")), te.test(d) || ne.test(d) || q.test(d) || ee.test(n))
              break;
...
` + d;
            ...
            !f && !d.trim() && (f = !0), l += Z + `
`, n = n.substring(Z.length + 1), h = d.slice(w);
          }
        }
        r.loose || (p ? r.loose = !0 : /\n *\n *$/.test(l) && (p = !0));
        let b = null, $;
        this.options.gfm && (b = /^\[[ xX]\] /.exec(s), b && ($ = b[0] !== "[ ] ", s = s.replace(/^\[[ xX]\] +/, ""))), r.items.push({
          type: "list_item",
          raw: l,
          task: !!b,
          checked: $,
...
const Se = /^(?: *(?:\n|$))+/, Ae = /^( {4}[^\n]+(?:\n(?: *(?:\n|$))*)?)+/, Re = /^ {0,3}(`{3,}(?=[^`\n]*(?:\n|$))|~{3,})([^\n]*)(?:\n|$)(?:|([\s\S]*?)(?:\n|$))(?: {0,3}\1[~`]* *(?=\n|$)|$)/, I = /^ {0,3}((?:-[\t ]*){3,}|(?:_[ \t]*){3,}|(?:\*[ \t]*){3,})(?:\n+|$)/, Me = /^ {0,3}(#{1,6})(?=\s|$)(.*)(?:\n+|$)/, he = /(?:[*+-]|\d{1,9}[.)])/, fe = m(/^(?!bull |blockCode|fences|blockquote|heading|html)((?:.|\n(?!\s*?\n|bull |blockCode|fences|blockquote|heading|html))+?)\n {0,3}(=+|-+) *(?:\n+|$)/).replace(/bull/g, he).replace(/blockCode/g, / {4}/).replace(/fences/g, / {0,3}(?:`{3,}|~{3,})/).replace(/blockquote/g, / {0,3}>/).replace(/heading/g, / {0,3}#{1,6}/).replace(/html/g, / {0,3}<[^\n>]+>\n/).getRegex(), F = /^([^\n]+(?:\n(?!hr|heading|lheading|blockquote|fences|list|html|table| +\n)[^\n]+)*)/, Pe = /^[^\n]+/, U = /(?!\s*\])(?:\\.|[^\[\]\\])+/, Le = m(/^ {0,3}\[(label)\]: *(?:\n *)?([^<\s][^\s]*|<.*?>)(?:(?: +(?:\n *)?| *\n *)(title))? *(?:\n+|$)/).replace("label", U).replace("title", /(?:"(?:\\"?|[^"\\])*"|'[^'\n]*(?:\n[^'\n]+)*\n?'|\([^()]*\))/).getRegex(), De = m(/^( {0,3}bull)([ \t][^\n]+?)?(?:\n|$)/).replace(/bull/g, he).getRegex(), O = "address|article|aside|base|basefont|blockquote|body|caption|center|col|colgroup|dd|details|dialog|dir|div|dl|dt|fieldset|figcaption|figure|footer|form|frame|frameset|h[1-6]|head|header|hr|html|iframe|legend|li|link|main|menu|menuitem|meta|nav|noframes|ol|optgroup|option|p|param|search|section|summary|table|tbody|td|tfoot|th|thead|title|tr|track|ul", J = /<!--(?:-?>|[\s\S]*?(?:-->|$))/, Oe = m("^ {0,3}(?:<(script|pre|style|textarea)[\\s>][\\s\\S]*?(?:</\\1>[^\\n]*\\n+|$)|comment[^\\n]*(\\n+|$)|<\\?[\\s\\S]*?(?:\\?>\\n*|$)|<![A-Z][\\s\\S]*?(?:>\\n*|$)|<!\\[CDATA\\[[\\s\\S]*?(?:\\]\\]>\\n*|$)|</?(tag)(?: +|\\n|/?>)[\\s\\S]*?(?:(?:\\n *)+\\n|$)|<(?!script|pre|style|textarea)([a-z][\\w-]*)(?:attribute)*? */?>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n *)+\\n|$)|</(?!script|pre|style|textarea)[a-z][\\w-]*\\s*>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n *)+\\n|$))", "i").replace("comment", J).replace("tag", O).replace("attribute", / +[a-zA-Z:_][\w.:-]*(?: *= *"[^"\n]*"| *= *'[^'\n]*'| *= *[^\s"'=<>`]+)?/).getRegex(), ge = m(F).replace("hr", I).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("|lheading", "").replace("|table", "").replace("blockquote", " {0,3}>").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)]) ").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", O).getRegex(), qe = m(/^( {0,3}> ?(paragraph|[^\n]*)(?:\n|$))+/).replace("paragraph", ge).getRegex(), W = {
  ...
  table: _,
  text: Pe
}, ce = m("^ *([^\\n ].*)\\n {0,3}((?:\\| *)?:?-+:? *(?:\\| *:?-+:? *)*(?:\\| *)?)(?:\\n((?:(?! *\\n|hr|heading|blockquote|code|fences|list|html).*(?:\\n|$))*)\\n*|$)").replace("hr", I).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("blockquote", " {0,3}>").replace("code", " {4}[^\\n]").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)]) ").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", O).getRegex(), Ze = {
  ...W,
  table: ce,
  paragraph: m(F).replace("hr", I).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("|lheading", "").replace("table", ce).replace("blockquote", " {0,3}>").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)]) ").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", O).getRegex()
}, je = {
  ...W,
...
class y {
  constructor(n) {
    ...
    this.options.pedantic ? (e.block = M.pedantic, e.inline = z.pedantic) : this.options.gfm && (e.block = M.gfm, this.options.breaks ? e.inline = z.breaks : e.inline = z.gfm), this.tokenizer.rules = e;
  }
  /**
   * Expose Rules
   */
   ...
   */
  static lex(n, e) {
    return new y(e).lex(n);
  }
  /**
   * Static Lex Inline Method
   */
  static lexInline(n, e) {
    return new y(e).inlineTokens(n);
  }
  /**
   * Preprocessing
   */
...
class v {
  constructor(n) {
    ...
    x(this, "renderer");
    x(this, "textRenderer");
    this.options = n || N, this.options.renderer = this.options.renderer || new D(), this.renderer = this.options.renderer, this.renderer.options = this.options, this.textRenderer = new X();
  }
  /**
   * Static Parse Method
   */
  static parse(n, e) {
    return new v(e).parse(n);
  }
  /**
   * Static Parse Inline Method
   */
  static parseInline(n, e) {
    return new v(e).parseInline(n);
  }
  /**
   * Parse Loop
   */
  parse(n, e = !0) {
    ...
    for (let i = 0; i < n.length; i++) {
      ...
      switch (r.type) {
        ...
        case "heading": {
          const o = r;
          t += this.renderer.heading(this.parseInline(o.tokens), o.depth, _e(this.parseInline(o.tokens, this.textRenderer)));
          continue;
        }
        case "code": {
          const o = r;
        ...
        case "list": {
          ...
          for (let h = 0; h < o.items.length; h++) {
            ...
            if (d.task) {
              ...
              p ? d.tokens.length > 0 && d.tokens[0].type === "paragraph" ? (d.tokens[0].text = $ + " " + d.tokens[0].text, d.tokens[0].tokens && d.tokens[0].tokens.length > 0 && d.tokens[0].tokens[0].type === "text" && (d.tokens[0].tokens[0].text = $ + " " + d.tokens[0].tokens[0].text)) : d.tokens.unshift({
                type: "text",
                text: $ + " "
              }) : b += $ + " ";
            }
...
class rt {
  constructor(...n) {
    ...
    x(this, "parse", A(this, T, Q).call(this, y.lex, v.parse));
    x(this, "parseInline", A(this, T, Q).call(this, y.lexInline, v.parseInline));
    x(this, "Parser", v);
    x(this, "Renderer", D);
    x(this, "TextRenderer", X);
    x(this, "Lexer", y);
    x(this, "Tokenizer", P);
    x(this, "Hooks", C);
    this.use(...n);
    ...
    return n.forEach((t) => {
      ...
      if (t.hooks) {
        ...
        for (const o in t.hooks) {
          ...
          C.passThroughHooks.has(o) ? r[l] = (u) => {
...
g.options = g.setOptions = function(a) {
  return E.setOptions(a), g.defaults = E.defaults, ue(g.defaults), g;
};
g.getDefaults = K;
g.defaults = N;
g.use = function(...a) {
  return E.use(...a), g.defaults = E.defaults, ue(g.defaults), g;
};
g.walkTokens = function(a, n) {
  return E.walkTokens(a, n);
};
g.parseInline = E.parseInline;
g.Parser = v;
g.parser = v.parse;
g.Renderer = D;
g.TextRenderer = X;
...
async function lt(a, n) {
  ...
  try {
    ...
    j(t, { response: u, status: "success" });
  } catch (s) {
    s.name === "AbortError" ? j(t, { response: "Запрос отменен.", status: "aborted" }) : j(t, { response: `Ошибка: ${s.message}`, status: "error" });
  } finally {
...
const at = {
  ...
  key: 0,
  class: "ai-dropdown"
}, bt = ["onClick"], Vt = { class: "ai-dropdown-wrap" }, yt = { class: "tool-text" }, vt = {
  key: 0,
  class: "ai-dropdown"
}, $t = ["onClick"], Et = { class: "ai-tools-right" }, Tt = /* @__PURE__ */ window.Vue.defineComponent({
  __name: "ai-chat",
  setup(a) {
            ...
            window.Vue.createElementVNode("div", gt, [
              ...
              window.Vue.createElementVNode("div", Vt, [
                ...
                t.value ? (window.Vue.openBlock(), window.Vue.createElementBlock("div", vt, [
                  ...
                  }, window.Vue.toDisplayString(f), 11, $t))), 128))
                ])) : window.Vue.createCommentVNode("", !0)
              ])
            ]),
...
}), Nt = { class: "ai-tab-view" }, zt = { class: "ai-body custom-scrollbar" }, _t = { class: "ai-settings" }, Ct = { class: "settings-block" }, Bt = { class: "settings-block" }, It = {
  key: 0,
  class: "prompt-editor"
}, St = { class: "editor-actions" }, At = { class: "prompts-list" }, Rt = { class: "prompt-info" }, Mt = { class: "prompt-name" }, Pt = { class: "prompt-preview" }, Lt = { class: "prompt-actions" }, Dt = ["onClick"], Ot = ["onClick"], qt = /* @__PURE__ */ window.Vue.defineComponent({
  __name: "ai-settings",
  setup(a) {
            ...
            window.Vue.createElementVNode("div", At, [
              (window.Vue.openBlock(!0), window.Vue.createElementBlock(window.Vue.Fragment, null, window.Vue.renderList(window.Vue.unref(c).systemPrompts, (p) => (window.Vue.openBlock(), window.Vue.createElementBlock("div", {
                ...
                window.Vue.createElementVNode("div", Rt, [
                  window.Vue.createElementVNode("div", Mt, window.Vue.toDisplayString(p.name), 1),
...
}), Zt = { class: "ai-tab-view" }, jt = { class: "ai-body custom-scrollbar" }, Ht = { class: "topics-header" }, Qt = {
  key: 0,
  class: "topics-empty"
}, Kt = { class: "topics-list" }, Ft = ["onClick"], Ut = { class: "topic-info" }, Jt = { class: "topic-title" }, Wt = { class: "topic-meta" }, Gt = ["onClick"], Xt = /* @__PURE__ */ window.Vue.defineComponent({
  __name: "ai-topics",
  setup(a) {
    function n(e) {
    ...
    return (e, t) => (window.Vue.openBlock(), window.Vue.createElementBlock("div", Zt, [
        ...
        window.Vue.createElementVNode("div", Kt, [
          (window.Vue.openBlock(!0), window.Vue.createElementBlock(window.Vue.Fragment, null, window.Vue.renderList(window.Vue.unref(c).topics, (i) => (window.Vue.openBlock(), window.Vue.createElementBlock("div", {
            ...
            window.Vue.createElementVNode("div", Ut, [
              window.Vue.createElementVNode("div", Jt, window.Vue.toDisplayString(i.title), 1),
              window.Vue.createElementVNode("div", Wt, [
  ...
  stroke: "currentColor",
  "stroke-width": "2"
}, nn = { class: "ai-modal" }, on = { class: "ai-header" }, rn = { class: "ai-header-actions" }, sn = { class: "ai-tabs" }, ln = { class: "ai-tabs-content" }, an = /* @__PURE__ */ window.Vue.defineComponent({
  ...
  setup(a) {
    return window.Vue.onMounted(() => st()), (n, e) => (window.Vue.openBlock(), window.Vue.createElementBlock(window.Vue.Fragment, null, [
      window.Vue.createElementVNode("button", window.Vue.mergeProps(n.$attrs, {
        ...
        e[8] || (e[8] = window.Vue.createStaticVNode('<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 8V4H8"></path><rect x="4" y="8" width="16" height="12" rx="2"></rect><path d="M2 14h2"></path><path d="M20 14h2"></path><path d="M15 13v2"></path><path d="M9 13v2"></path></svg>', 1)),
        window.Vue.unref(c).isLoading && !window.Vue.unref(c).isOpen ? (window.Vue.openBlock(), window.Vue.createElementBlock("div", Yt)) : window.Vue.createCommentVNode("", !0)
      ], 16),
      (window.Vue.openBlock(), window.Vue.createBlock(window.Vue.Teleport, { to: "body" }, [
        window.Vue.unref(c).isMinimized ? (window.Vue.openBlock(), window.Vue.createElementBlock("div", {
          ...
          window.Vue.unref(c).isLoading ? (window.Vue.openBlock(), window.Vue.createElementBlock("div", en)) : (window.Vue.openBlock(), window.Vue.createElementBlock("svg", tn, [...e[9] || (e[9] = [
        ...
        window.Vue.createVNode(window.Vue.Transition, { name: "ai-fade" }, {
          default: window.Vue.withCtx(() => [
            window.Vue.unref(c).isOpen ? (window.Vue.openBlock(), window.Vue.createElementBlock("div", {
              key: 0,
              class: "ai-backdrop",
              onMousedown: e[7] || (e[7] = window.Vue.withModifiers((t) => window.Vue.unref(k).close(), ["self"]))
            }, [
              window.Vue.createElementVNode("div", nn, [
                window.Vue.createElementVNode("div", on, [
                  e[12] || (e[12] = window.Vue.createElementVNode("div", { class: "ai-title" }, [
                    window.Vue.createElementVNode("svg", {
                      xmlns: "http://www.w3.org/2000/svg",
                      width: "20",
                      height: "20",
                  ...
                  window.Vue.createElementVNode("div", rn, [
                    window.Vue.createElementVNode("button", {
                      ...
                      type: "button",
                      onClick: e[2] || (e[2] = window.Vue.withModifiers((t) => window.Vue.unref(k).minimize(), ["stop"]))
                    }, [...e[10] || (e[10] = [
                      window.Vue.createElementVNode("svg", {
                        xmlns: "http://www.w3.org/2000/svg",
                        width: "18",
                    ...
                    window.Vue.createElementVNode("button", {
                      ...
                      title: "Закрыть",
                      type: "button",
                      onClick: e[3] || (e[3] = window.Vue.withModifiers((t) => window.Vue.unref(k).close(), ["stop"]))
                    }, [...e[11] || (e[11] = [
                      window.Vue.createElementVNode("svg", {
                        xmlns: "http://www.w3.org/2000/svg",
                        width: "18",
                ...
                window.Vue.createElementVNode("div", ln, [
                  ...
                  window.Vue.unref(c).activeTab === "topics" ? (window.Vue.openBlock(), window.Vue.createBlock(Xt, { key: 0 })) : window.Vue.createCommentVNode("", !0),
...
.ai-modal { width: 100%; max-width: 680px; height: 85vh; max-height: 800px; background: var(--bg-primary-color); border: 1px solid var(--border-secondary-color); border-radius: 16px; box-shadow: 0 24px 48px -12px rgba(0, 0, 0, 0.3); display: flex; flex-direction: column; overflow: hidden; }
...
/* === СКРОЛЛЫ И ТЕЛО === */
.ai-body { flex: 1; overflow-y: auto; padding: 20px; display: flex; flex-direction: column; gap: 20px; }
...
.ai-response-bubble { color: var(--fg-primary-color); background: var(--bg-secondary-color); border: 1px solid var(--border-secondary-color); padding: 14px 18px; border-radius: 4px 18px 18px 18px; align-self: flex-start; max-width: 90%; font-size: 0.95rem; line-height: 1.6; }
...
.ai-input-bottom { position: absolute; bottom: 8px; left: 8px; right: 8px; display: flex; justify-content: space-between; align-items: center; }
.ai-tools-left { display: flex; gap: 4px; align-items: center; }

/* Кнопки Тулбара */
.ai-tool-btn { display: flex; align-items: center; gap: 6px; background: transparent; border: none; color: var(--fg-secondary-color); padding: 6px 8px; border-radius: 8px; cursor: pointer; transition: all 0.2s; font-size: 0.8rem; }
.ai-tool-btn:hover { background: var(--bg-hover-color); color: var(--fg-primary-color); }
.tool-text { max-width: 90px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; font-weight: 500; }
...
/* === ТОПИКИ === */
.topics-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px; }
...
.ai-settings { display: flex; flex-direction: column; gap: 24px; }
